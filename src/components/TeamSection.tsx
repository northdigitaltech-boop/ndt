  ...
  return (
    <section id="team" className="relative bg-[#07111f] py-28 px-6 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {featuredOnly && visibleTeam.length === 1 ? (
          /* Single featured member (home page): horizontal layout — photo + role left, details right */
          <div className="max-w-4xl mx-auto">
            {visibleTeam.map((member) => (
              <motion.div
                ...
                className="relative bg-[#0a1628]/80 backdrop-blur-sm border border-cyan-500/15 hover:border-cyan-500/40 rounded-2xl p-8 md:p-10 flex flex-col md:flex-row items-center md:items-center gap-8 shadow-xl hover:shadow-cyan-500/15 hover:shadow-2xl transition-all duration-300 group overflow-hidden"
              >
                <div className="absolute inset-0 bg-linear-to-r from-cyan-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl" />

                {/* Left: photo + role */}
                <div className="relative flex flex-col items-center gap-4 shrink-0">
                  <div className="relative w-44 h-44 rounded-full p-0.5 bg-linear-to-br from-cyan-400 to-blue-600 shadow-lg shadow-cyan-500/30 group-hover:shadow-cyan-500/50 transition-all duration-300">
                    <div className="relative w-full h-full rounded-full overflow-hidden">
                      <Image src={member.image} alt={member.name} fill sizes="176px" className={`object-cover ${member.pos}`} />
                    </div>
                  </div>
                  <span className="inline-block px-3 py-1 bg-cyan-500/10 border border-cyan-500/30 rounded-full text-cyan-400 text-xs font-semibold tracking-wide">
                    {member.role}
                  </span>
                </div>
                ...
                <div className="relative text-center md:text-left">
                  ...
                  <p className="mt-4 text-gray-400 text-base leading-relaxed">{member.bio}</p>
                </div>
              </motion.div>
            ))}
          </div>
